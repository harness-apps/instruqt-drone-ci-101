...
As part of this tutorial we have an `$EXAMPLES_HOME`(examples) folder that has various exercise sources/demos.
...
Open the exercise folder **$EXAMPLES_HOME/greeting** on the **Editor** tab and analyse the Drone pipeline. It is simple greeter application that says **hello world** in multiple languages.
...
> **NOTE:**
> As you noticed we used new configuration block in the pipeline called *services*. Services are detached containers that are not part of your step but will be used by steps. These containers are killed automatically after the pipeline execution ends.
